 ...
 */ 
	class GM_Meta_Box{
		...
		function show() {
			...
			echo '<table class="form-table gm-metabox">';
				...
				echo '<tr id="'.$field['id'].'_box">';
					...
					switch($field['type']) {
						...
						case 'rating_criteria':
							...
							if ( count( $rows ) > 0 ) {
								foreach( $rows as $row ) {
									if ( isset( $row['c_label'] ) || isset( $row['score'] ) ) {
										echo '
										<p>
										<label for="'.$field['id'].'['.$c.'][c_label]">Kriter:</label> 
										<input type="text" name="'.$field['id'].'['.$c.'][c_label]" value="'.$row['c_label'].'" />
										<label for="'.$field['id'].'['.$c.'][score]">Skor:</label> 
										<input type="text" name="'.$field['id'].'['.$c.'][score]" value="'.$row['score'].'" />
										<a class="remove button-secondary">Kaldır</a>
										</p>';
										$c = $c + 1;
									}
								}
							...
							echo '<a class="add-criteria button-default" href="#">Yeni Kriter Ekle</a>';
							echo '<br /><span style="margin-top: 10px; display: block;" class="description">'.$field['desc'].'</span>';
							?>
							<script>
...
/*	Initialize Metabox
 *	--------------------------------------------------------- */
	function gm_init_metaboxes() {
		if ( class_exists( 'GM_Meta_Box' ) ) {
			require_once(TEMPLATEPATH. '/framework/metaboxes/gm-metaboxes.php');
			$metaboxes = array();
			$metaboxes = apply_filters ( 'gm_metaboxes' , $metaboxes );
			foreach ( $metaboxes as $metabox ) {
				$my_box = new GM_Meta_Box( $metabox );
			}
		}
	}
	add_action( 'init', 'gm_init_metaboxes', 9999 );
